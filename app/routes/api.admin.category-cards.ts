  ...
  try {
    ...
    switch (v.action) {
      case 'create': {
        ...
        auditLog({ action: 'settings_change', role, resource: `metaobject/${result.id}`, detail: `category_card_create${imgNotes.length ? '; ' + imgNotes.join('; ') : ''}`, success: true, ...diff });
      ...
      case 'update': {
        ...
        auditLog({ action: 'settings_change', role, resource: `metaobject/${v.metaobjectId}`, detail: `category_card_update${imgNotes.length ? '; ' + imgNotes.join('; ') : ''}`, success: true, ...diff });
        return data({ success: true, metaobject: result, imageNotes: imgNotes });
      }
      ...
      case 'delete': {
        ...
        auditLog({ action: 'settings_change', role, resource: `metaobject/${v.metaobjectId}`, detail: 'category_card_delete', success: result, ...diff });
