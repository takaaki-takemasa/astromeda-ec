...
// patch 0125 Phase C: ファネル（来訪→商品→カート→購入手続き の各段階で何 % 落ちたか）
const AdminFunnel = lazy(() => import('~/components/admin/tabs/AdminFunnel'));
    ...
    >
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden' }}>
        ...
        <main style={{ flex: 1, padding: '24px 32px', overflow: 'auto' }}>
          ...
          {/* patch 0125 Phase C: ファネル（来訪→商品→カート→購入手続き 離脱率の見える化） */}
          {subTab === 'funnel' && (
            <Suspense fallback={<TabLoadingSkeleton />}>
              <AdminFunnel />
            </Suspense>
