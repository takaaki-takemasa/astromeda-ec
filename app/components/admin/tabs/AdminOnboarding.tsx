...
const STEPS: Step[] = [
  {
    ...
    detail: [
      ...
      '3. URL 末尾は空欄でも自動生成される。指定するなら半角英数で短く。例: one-piece-collaboration',
  ...
  },
  {
    ...
    detail: [
      ...
      '4. ステータスを「🟢 公開」にして保存（下書き保存したい場合は「📝 下書き」）。',
      '5. 保存後、商品詳細編集画面の「バリアント」タブからバリエーション（メモリ違い等）の在庫数や価格をその場で編集できる。',
    ],
    cautions: [
  ...
  },
  {
    ...
    detail: [
      ...
      '2. 「＋ 新規追加」を押し、表示名／ Shopify コレクション URL 末尾（ステップ①で控えたもの）／表示順／「フロントに表示する」トグル を入力。',
  ...
  },
  {
    ...
    cautions: [
      '登録データが 1 件もない場合は、初期値の標準オプション（メモリ／ SSD ×2 ／ HDD ／電源 …の 17 項目）が表示される。完全置き換えしたい場合は、ここで全項目を登録する必要がある。',
  ...
  },
  {
    ...
    cautions: [
      ...
      'カスタマイズが出ない場合は、商品タグに pc/gaming が無いか、登録データの applies_to_tags 設定がずれている。',
    ],
    links: [
      {
        label: '本番サイト トップを開く',
