...
const STEPS: Step[] = [
  {
    ...
    summary: 'Shopify 側で IP 用のコレクションを作成し、商品をタグで紐付ける土台を作る。',
    detail: [
      '1. Shopify 管理画面 → 「商品管理」 → 「コレクション」 → 「コレクションを作成」を押す。',
      '2. タイトルにコラボ名（例: "ワンピース"）を入力。ハンドル（URL）は半角英数で短く。例: one-piece-collaboration',
      '3. 「コレクションの種類」で「自動」を選び、条件 = 「商品タグ」 「次と等しい」 「one-piece」 を設定（タグ名は任意）。',
      '4. ヒーローバナー画像（1920×600px 推奨）をコレクション画像にアップロード。',
      '5. 保存して、コレクションのハンドル（URL）を控える。次のステップで使う。',
    ],
    cautions: [
    ...
    links: [
      {
        label: 'Shopify でコレクションを作る',
        type: 'shopify',
        target: '/collections/new',
        description: 'Shopify 標準のコレクション作成画面が開く',
      },
      {
        label: 'Shopify コレクション一覧',
        type: 'shopify',
        target: '/collections',
      },
    ],
  ...
  },
  {
    ...
    summary: 'タグが正しければ自動コレクションが拾うため、Shopify 管理画面で確認するだけ。',
    detail: [
      '1. ステップ② で付けたタグが、ステップ① のコレクション条件と一致しているか確認する。',
      '2. Shopify 管理画面で対象コレクションを開き、「商品」セクションに新製品が表示されていれば成功。',
      '3. もし表示されない場合は、商品のタグを修正するか、コレクションの条件（タグ名のスペル）を見直す。',
      '4. 大量に追加する場合は、Shopify の「一括編集」機能でタグを一括付与すると速い。',
    ],
    cautions: [
      '手動コレクション（条件なし）に切り替えると自動拾いが効かなくなる。基本は「自動 + タグ条件」で運用する。',
    ],
    links: [
      {
        label: 'Shopify コレクション一覧',
