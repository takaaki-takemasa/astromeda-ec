...
export default function AdminMenus() {
  ...
  const handleDelete = useCallback(
    async (m: MenuSummary) => {
      ...
      const res = await apiAction({action: 'delete', id: m.id, confirm: true});
      if (res.success) {
        showToast('削除しました', 'ok');
        reload();
      } else {
  ...
  return (
    ...
    <TabHeaderHint
      title="お客様向けの道案内（メニュー）"
      description="ヘッダーやフッターに表示されるメニュー項目（「ゲーミングPC」「会社情報」など）を編集します。"
      relatedTabs={[{label: 'ページの引っ越し転送', tab: 'redirects'}, {label: 'お店の見た目を変える', tab: 'pageEditor'}]}
    />
      {/* Header */}
      <div
        style={{
